...
taxon_density <- taxon_count_data %>% 
  ...
  mutate(taxon_verified = case_when(taxon == "mussel" ~ "mytilus", 
                                    ...
                                    taxon == "tunicate" ~ "clear_tunicates",
...
plotting_names <- mean_density_stage %>% 
  ...
  mutate(plotting_name = case_when(is.na(plotting_name) ~ Phylum,
                                   TRUE ~ plotting_name)) %>% 
  mutate(plotting_name = case_when(taxon_name == "red-eye amphipod" ~ "Amphipoda indet 1",
                                   taxon_name == "purple amphipod" ~ "Amphipoda indet 2",
                                   taxon_name == "clear_tunicates" ~ "Tunicata indet 1",
                                   taxon_name == "colonial tunicate" ~ "Tunicata indet 2",
                                   taxon_name == "crustacean larvae" ~ "Crustacea larvae indet",
                                   TRUE ~ plotting_name)) %>%
...
mean_density_stage_plotting <- plotting_names %>% 
...
species_pos_table <- data.frame(taxon = species_names) %>%
  dplyr::mutate(x_center = (1:n()), 
         width = 1) %>% 
  left_join(plotting_names %>% rename("taxon" = taxon_name) %>% select(-stage, -avg_density), by = "taxon") %>% 
  mutate(plotting_name = case_when(taxon == "red_eye_amphipod" ~ "Amphipoda indet 1",
                                   taxon == "purple_amphipod" ~ "Amphipoda indet 2",
                                   taxon == "colonial_tunicate" ~ "Tunicata indet 2",
                                   taxon == "tunicata" ~ "Tunicata indet 3",
                                   taxon == "crustacean_larvae" ~ "Crustacea larvae indet",
                                   taxon == "clear_tunicates" ~ "Tunicata indet 2", 
                                   !is.na(plotting_name) ~ plotting_name),
         Phylum = case_when(taxon == "red_eye_amphipod" ~ "Arthropoda",
                                   taxon == "purple_amphipod" ~ "Arthropoda",
                                   taxon == "colonial_tunicate" ~ "Chordata",
                                   taxon == "crustacean_larvae" ~ "Arthropoda",
                                   taxon == "clear_tunicates" ~ "Chordata", 
                                   !is.na(Phylum) ~ Phylum),
         labs_colour = case_when(Phylum == "Annelida" ~ pal[1],
...
plt_hmap <- ggplot(heatmap_data, 
  ...
  scale_x_continuous(breaks = species_pos_table$x_center, 
                     labels = species_pos_table$plotting_name,
                     expand = c(0, 0)
                     ) + 
...
Highest densities
```{r}
max(taxon_density$avg_density, na.rm = TRUE)
taxon_density %>% 
  group_by(id, taxon_verified, area, stage) %>% 
  summarise(avg_density = sum(avg_density)) %>%
