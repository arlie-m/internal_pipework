...
save_plot(here("outputs", "nmds_plot.png"), nmds_plot, base_width = 183, base_height = 160, units = "mm")
```
Checking the nMDS
...
mean_density_stage_plotting <- mean_density_stage %>% 
  ...
  mutate(plotting_name = case_when(is.na(Family) ~ Order,
                                   TRUE ~ plotting_name)) %>%
  mutate(plotting_name = case_when(is.na(Order) ~ Class,
                                   TRUE ~ plotting_name)) %>%
  mutate(plotting_name = case_when(is.na(Class) ~ Subphylum,
                                   TRUE ~ plotting_name)) %>%
  mutate(plotting_name = case_when(is.na(Subphylum) ~ Phylum,
                                   TRUE ~ plotting_name)) %>% 
...
ggplot() +
  geom_bar(data = mean_density_stage_plotting,
           aes(x = stage, y = avg_density, fill = plotting_name),
           stat = "identity",
           position = "stack") +
...
plt_hmap <- ggplot(heatmap_data, 
  ...
  geom_tile() +
  scale_fill_viridis_c("Log density", option = "cividis", na.value = "white", begin = 0.2) +
